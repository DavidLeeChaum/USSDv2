...
const ICT = artifacts.require('SStableICT');
...
contract('SStable', async function (accounts) {
  ...
  */
  ...
  it('connects staking and insurance contracts', async function() {
    this.stSStable = await stSStable.new(this.SStable.address, "Staked SStable", "stStable", { from: accounts[0] });
    this.ICT = await ICT.new(this.SStable.address, "SStable Insurance Capital Trust", "SStable ICT", { from: accounts[0] });

    await truffleAssert.reverts(this.SStable.connectStaking(this.stSStable.address, { from: accounts[1] }), "owner");
    await truffleAssert.reverts(this.SStable.connectInsurance(this.ICT.address, { from: accounts[1] }), "owner");

    await this.SStable.connectStaking(this.stSStable.address);
    await this.SStable.connectInsurance(this.ICT.address);
    
    await truffleAssert.reverts(this.SStable.connectStaking(this.stSStable.address, { from: accounts[1] }), "");
    await truffleAssert.reverts(this.SStable.connectInsurance(this.ICT.address, { from: accounts[1] }), "");
  });

  it('can switch to WETH', async function() {

    await truffleAssert.reverts(this.SStable.switchToWETH({ from: accounts[1] }), "owner");

    await this.SStable.switchToWETH();

    await truffleAssert.reverts(this.SStable.switchToWETH({ from: accounts[0] }), "");
  });

  it('can change owner', async function() {
    await truffleAssert.reverts(this.SStable.changeOwner(accounts[2], { from: accounts[1] }), "owner");

    await this.SStable.changeOwner(accounts[2], { from: accounts[0] });

    await truffleAssert.reverts(this.SStable.changeOwner(accounts[3], { from: accounts[0] }), "owner");

    await this.SStable.changeOwner(accounts[3], { from: accounts[2] });
  });
  ...
  it('can mint itself as reward for staking or insurance contract', async function() {
    ...
    await this.SStable.connectStaking(accounts[2]);
    await this.SStable.connectInsurance(accounts[3]);

    await truffleAssert.reverts(this.SStable.mintRewards(web3.utils.toBN('1000'), accounts[1], { from: accounts[0] }), "minter");

    await this.SStable.mintRewards(web3.utils.toBN('1000'), accounts[1], { from: accounts[2] });
    await this.SStable.mintRewards(web3.utils.toBN('2500'), accounts[1], { from: accounts[3] });

    expect((await this.SStable.balanceOf(accounts[1])).toString()).to.equal('1733'); // 1750 - 1% founder fee
  });
  ...
  it('can be minted for token (stables, WETH)', async function() {
    ...
    await truffleAssert.reverts(this.SStable.mintForToken(WETH, web3.utils.toBN('1000000000000000000'), accounts[1], { from: accounts[0] }), "USSD only");
    
    await this.SStable.mintForToken(USSD, web3.utils.toBN('1000000'), accounts[1], { from: accounts[0] });
    console.log("Minted 0.5 SStable for 1.0 USSD");

    expect((await this.SStable.balanceOf(accounts[1])).toString()).to.equal('500000');
    ...
    await this.SStable.mintForToken(WETH, web3.utils.toBN('100000000000000000'), accounts[2], { from: accounts[0] });
    console.log("Minted 125.0 SStable for 0.1 WETH");

    expect((await this.SStable.balanceOf(accounts[2])).toString()).to.equal('125000000');

    expect((await this.SStable.totalSupply()).toString()).to.equal('125500000');
  ...
  it('can be redeemed for stablecoins', async function() {
    ...
    expect((await this.SStable.totalSupply()).toString()).to.equal('5000000');

    expect((await this.SStable.collateralFactor()).toString()).to.equal('1000000000000000000');

    // no SStable to redeem reverts
    await truffleAssert.reverts(this.SStable.redeem(web3.utils.toBN('7500000'), accounts[2], { from: accounts[0] }));
    
    await this.SStable.redeem(web3.utils.toBN('3750000'), accounts[2], { from: accounts[1] });

    expect((await this.SStable.totalSupply()).toString()).to.equal('1250000');
    expect((await USSDContract.methods.balanceOf(accounts[2]).call()).toString()).to.equal('7500000');
  });

  it('can be redeemed for stablecoins and other collateral (WETH)', async function() {
    await prepareAssetsSStable(accounts);
    ...
    await this.SStable.redeem(web3.utils.toBN('15000000'), accounts[3], { from: accounts[1] });

    expect((await this.SStable.totalSupply()).toString()).to.equal('2500000');

    // we expect both USSD and WETH to be given on redeem
    expect((await USSDContract.methods.balanceOf(accounts[3]).call()).toString()).to.equal('10000000');
    expect((await WETHContract.methods.balanceOf(accounts[3]).call()).toString()).to.equal('8000000000000000');
  });
/*
  it('can be minted for WBTC and redeemed with penalty when collateral factor drops', async function() {
    ...
    // we expect both USDT and WBTC to be given on redeem
    expect((await USDTContract.methods.balanceOf(accounts[4]).call()).toString()).to.equal('0'); // no stables left
