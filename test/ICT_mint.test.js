...
contract('ICT (insurance capital trust)', async function (accounts) {
  ...
  it('ICT could be minted for WBGL, pays USSD premiums', async function() {
    ...
    await truffleAssert.reverts(this.USSD.mintForToken(WETH, web3.utils.toBN('100000000'), accounts[1], { from: accounts[0] }), "STABLE only");
    
    await this.USSD.mintForToken(USDT, web3.utils.toBN('100000000'), accounts[2], { from: accounts[0] });
    ...
    // 0.1 WBGL
    await this.ICT.mintWBGL(web3.utils.toBN('100000000000000000'), accounts[0], { from: accounts[0] });
    ...
    expect((await this.ICT.balanceOf(accounts[0])).toString()).to.equal('100000000000000000');
    ...
    // add second staker
    await WBGLContract.methods.transfer(accounts[1], web3.utils.toBN('100000000000000000')).send({ from: accounts[0] });
    await WBGLContract.methods.approve(this.ICT.address, web3.utils.toBN('1000000000000000000000')).send({ from: accounts[1] });

    expect((await this.ICT.totalSupply()).toString()).to.equal('100000000000000000');
    await this.ICT.mintWBGL(web3.utils.toBN('100000000000000000'), accounts[1], { from: accounts[1], gas: 5000000 });
    expect((await this.ICT.totalSupply()).toString()).to.equal('200000000000000000');
