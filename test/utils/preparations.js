...
const prepareAssetsSStable = async (accounts, buyWBGL, buyUSSD) => {
  ...
  if(buyUSSD) {
      ...
      console.log("Calculated route to swap WETH to USSD");
      ...
      console.log("Swapped USDT to USSD");
  }
  ...
  if (buyUSSD) {
      await USSDContract.methods.balanceOf(accounts[0]).call(function(error, result) {
      console.log(`USSD balance of ${accounts[0]}: ${result}`);
      });
  }
}
...
const prepareOraclesSStable = async (that, accounts) => {
  that.oracleAsset = await SimOracle.new(web3.utils.toBN('2000000000000000000'), { from: accounts[0] });
  ...
  await that.SStable.setOracles(that.oracleAsset.address, that.oracleUSDT.address, that.oracleWBGL.address, that.oracleWBTC.address, that.oracleWETH.address);
};

module.exports = { prepareAssets, prepareAssetsSStable, prepareOracles, prepareOraclesSStable, prepareInsuranceAssets };