...
import { SafeTransferLib, ERC20, SStableRewards } from "./SStableRewards.sol";
import { FixedPointMathLib } from "solmate/src/utils/FixedPointMathLib.sol";

import "./interfaces/IUSSDInsurance.sol";

contract SStableICT is SStableRewards, IUSSDInsurance {
    ...
    ) SStableRewards(_name, _symbol, 18, _USSD, SDT_RATE) {
    ...
    function mintWBGL(uint256 _amountWBGL, address receiver) public virtual returns (uint256 mintAmount) {
        ...
        require(ERC20(WBGL).balanceOf(address(this)) + _amountWBGL < 1000000 * 1e18, "quota reached");
    ...
    function mintWETH(uint256 _amountWETH, address receiver) public virtual returns (uint256 mintAmount) {
        require(switchedToWETH || ERC20(WBGL).balanceOf(address(this)) >= 1000000 * 1e18, "WBGL only");
